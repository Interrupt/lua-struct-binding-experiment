...
pub fn bindType(luaState: *zlua.Lua, comptime T: type, comptime metaTableName: [:0]const u8) !void {
    ...
    const gcFunc = struct {
        fn inner(L: *zlua.Lua) i32 {
            delve.debug.log("LuaType destroy called", .{});
        ...
        .@"struct" => |S| {
            ...
            inline for (decls) |decl| {
                if (@typeInfo(@TypeOf(@field(T, decl.name))) == .@"fn") {
                    luaState.pushClosure(zlua.wrap(bindStructFuncLua(T, @field(T, decl.name), metaTableName)), 0);
...
fn bindStructFuncLua(comptime T: type, comptime function: anytype, comptime metaTableName: [:0]const u8) fn (lua: *Lua) i32 {
    return (opaque {
        pub fn lua_call(luaState: *Lua) i32 {
            ...
            inline for (params, 0..) |param, i| {
                ...
                switch (param_type) {
                    *T => {
                        args[i] = luaState.checkUserdata(T, lua_idx, metaTableName);
                    },
                    bool => {
            ...
            switch (@TypeOf(ret_val)) {
                T => {
                    // make a new ptr
                    const ptr: *T = @alignCast(luaState.newUserdata(T, @sizeOf(T)));

                    // set its metatable
                    _ = luaState.getMetatableRegistry(metaTableName);
